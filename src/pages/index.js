...
import { apiFetch, setToken } from "../pages/api/api";
...
export default function Login() {
  ...
  const handleLogin = async () => {
    try {
      const { accessToken } = await apiFetch("/auth/dev-login", {
        method: "POST",
        body: { email, password }
      });
      ...
      <div className="p-8 sm:p-10 md:pt-12 md:pb-10 md:px-10 bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex flex-col items-center text-center space-y-6 sm:space-y-8">
          ...
          <div className="w-full space-y-3">
            <button
              className="w-full flex items-center justify-center gap-3 bg-white text-slate-700 px-5 py-3.5 rounded-xl border border-slate-200 hover:bg-slate-50 hover:border-slate-300 hover:shadow-sm transition-all duration-200 font-medium text-[15px]"
              type="button"
            >
              <img height="18" width="18" src="https://cdn.simpleicons.org/google" alt="Google" className="-ml-4" />
            ...
            >
              ...
              <button
                type="submit"
                className="inline-flex items-center justify-center gap-2 w-full h-11 sm:h-12 bg-slate-900 hover:bg-slate-800 text-white font-medium shadow-sm rounded-xl transition-all duration-200"
              >
                Sign in
              </button>
            </form>
